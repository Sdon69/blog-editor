...
app.get('/shopify/callback-request', (req, res) => {
 ...
 app.get('/shopify-redirects', (req, res) => {
  	console.log("shopify-request initiated");
...
const shop = req.query.shop;
  if (shop) {
    ...
    const state = nonce();
	console.log(forwardingAddress);
    const redirectUri = forwardingAddress + '/shopify/callback-redirects';
    const installUrl = 'https://' + shop +
      '/admin/oauth/authorize?client_id=' + apiKey +
      '&scope=' + scopes +
      '&state=' + state +
...
app.get('/shopify/callback-redirects', (req, res) => {
	console.log("callback-request-initiated");
  ...
  if (shop && hmac && code) {
    ...
    .then((accessTokenResponse) => {
      ...
      const shopRequestHeaders = {
	   ...
	   const shopRequestUrl = 'https://' + shop + '/admin/api/2019-07/redirects.json';
